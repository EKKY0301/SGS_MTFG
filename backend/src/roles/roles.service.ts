...
export class RolesService {
  ...
  async create(data: CreateRoleDto, userId: string) {
    ...
    await this.auditLogs.create({ userId, entity: 'Role', entityId: created.id, action: 'create', newValues: data as any });
    return created;
  }
  ...
  async update(id: string, data: UpdateRoleDto, userId: string) {
    const old = await this.prisma.role.findUnique({ where: { id } });
    const updated = await this.prisma.role.update({ where: { id }, data });
    await this.auditLogs.create({ userId, entity: 'Role', entityId: id, action: 'update', oldValues: old as any, newValues: data as any });
    return updated;
  }
  ...
  async delete(id: string, userId: string) {
    ...
    await this.auditLogs.create({ userId, entity: 'Role', entityId: id, action: 'delete', oldValues: old as any });
    return deleted;
  }
}
