import { Injectable } from '@nestjs/common';
import { PrismaService } from '../prisma/prisma.service';
import { AuditLogsService } from '../audit-logs/audit-logs.service';
import { CreateRentalDto } from './dto/create-rental.dto';
import { UpdateRentalDto } from './dto/update-rental.dto';
import { ListRequestDto } from '../shared/dto/query-list.dto';
import { SearchListDto } from '../shared/dto/search-list.dto';
import { PaginationDataDto } from '../shared/dto/pagination-data.dto';

@Injectable()
export class RentalsService {
  constructor(private prisma: PrismaService, private readonly auditLogs: AuditLogsService) {}

  private resolvePagination(pagination?: PaginationDataDto) {
    const page = Number(pagination?.page) || 1;
    const rawLimit = Number(pagination?.itemsPerPage) || 10;
    const limit = Math.min(10, Math.max(1, rawLimit));

    return {
      page,
      limit,
      skip: (page - 1) * limit,
    };
  }

  async create(data: CreateRentalDto, userId: string) {
    const created = await this.prisma.rental.create({ data });
    this.auditLogs.create({ userId, entity: 'Rental', entityId: created.id, action: 'create', newValues: data as any });
    return created;
  }

  findAll(query: ListRequestDto) {
    return this.search({
      paginationData: query,
      filters: {
        search: query.search,
        filter: query.filter,
      },
    });
  }

  async search(body: SearchListDto) {
    const { page, limit, skip } = this.resolvePagination(body.paginationData);
    const filters = body.filters ?? {};
    const where: any = {};

    if (filters.search) {
      where.OR = [
        { status: { contains: filters.search, mode: 'insensitive' } },
        { notes: { contains: filters.search, mode: 'insensitive' } },
      ];
    }

    if (filters.filter) {
      where.status = { contains: filters.filter, mode: 'insensitive' };
    }

    const total = await this.prisma.rental.count({ where });
    const data = await this.prisma.rental.findMany({
      where,
      skip,
      take: limit,
      orderBy: { createdAt: 'desc' },
    });

    return {
      data,
      page,
      limit,
      total,
      totalPages: Math.ceil(total / limit),
    };
  }

  findOne(id: string) {
    return this.prisma.rental.findUnique({ where: { id } });
  }

  async update(id: string, data: UpdateRentalDto, userId: string) {
    const old = await this.prisma.rental.findUnique({ where: { id } });
    const updated = await this.prisma.rental.update({ where: { id }, data });
    this.auditLogs.create({ userId, entity: 'Rental', entityId: id, action: 'update', oldValues: old as any, newValues: data as any });
    return updated;
  }

  async delete(id: string, userId: string) {
    const old = await this.prisma.rental.findUnique({ where: { id } });
    const deleted = await this.prisma.rental.delete({ where: { id } });
    this.auditLogs.create({ userId, entity: 'Rental', entityId: id, action: 'delete', oldValues: old as any });
    return deleted;
  }
}
